...
$('.check_status').click(function(){
	...
	$('#btn_simpan').click(function(){
			$.ajax({
				 url : '<?php echo base_url(); ?>stok_masuk/simpan',
				 data : $('#form_stok_masuk').serialize(),
				 type : "POST",
				 dataType : "json",
				 success : function(row){
					 window.location.href = '<?php echo base_url(); ?>stok_masuk';
				 }
			 });
			 e.preventDefault();
	 });
			 ...
			 swal({
					 ...
					 function(isConfirm) {
						 if (isConfirm) {
							 ...
							 $('#form_stok_masuk').prop('action', '<?php echo site_url();?>stok_masuk/tambah');
							 $("#form_stok_masuk").submit();
							 e.preventDefault();
						 } else {
							 swal("Batal", "Proses telah dibatalkan", "error");
							 $('#form_stok_masuk').prop('action', '<?php echo site_url();?>stok_masuk/tambah_tidak');
							 $("#form_stok_masuk").submit();
							 e.preventDefault();
						 }
					 });
	...
	function konfirmasi_timpa(){
		$.ajax({
				url : '<?php echo base_url(); ?>stok_masuk/tambah',
				data : $('#form_stok_masuk').serialize(),
				type : "POST",
				dataType : "json",
				success : function(row){
	...
	function simpan_biasa(){
		$.ajax({
				url : '<?php echo base_url(); ?>stok_masuk/tambah_biasa',
				data : $('#form_stok_masuk').serialize(),
				type : "POST",
				dataType : "json",
				success : function(row){
	...
	function konfirmasi_tidak(){
		$.ajax({
				url : '<?php echo base_url(); ?>stok_masuk/tambah_tidak',
				data : $('#form_stok_masuk').serialize(),
				type : "POST",
				dataType : "json",
				success : function(row){
  ...
  function tambah_bahan(){
    ...
    '<tr>'+
      ...
      '</td>'+
      '<td>'+
        '<input type="text" name="jumlah_beli[]" class="form-control" id="jumlah_beli_'+i+'" onkeyup="FormatCurrency(this); hitung_jumlah('+i+'); hitung_total();" required>'+
      '</td>'+
      '<td>'+
        '<div class="input-group">'+
          '<span class="input-group-addon">Rp</span>'+
					'<input type="text" name="harga_beli[]" class="form-control" id="harga_beli_'+i+'" onkeyup="FormatCurrency(this); hitung_jumlah('+i+'); hitung_total();" required>'+
        '</div>'+
      '</td>'+
      '<td>'+
        '<div class="input-group">'+
          '<span class="input-group-addon">Rp</span>'+
          '<input type="text" name="total_harga_beli[]" class="form-control class_harga_beli" id="total_harga_beli_'+i+'" onkeyup="FormatCurrency(this);" readonly required>'+
        '</div>'+
      '</td>'+
      '<td>'+
  ...
  function get_nama_bahan(number){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>stok_masuk/get_nama_bahan',
        data : {search:search},
        type : "POST",
        dataType : "json",
...
function klik_bahan(id, number){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>stok_masuk/klik_bahan',
        data : {id:id},
        type : "POST",
        dataType : "json",
        success : function(row){
                                            ...
                                            <table id="tabel_tambah_bahan" class="table table-striped table-bordered nowrap">
                                                ...
                                                <tbody>
                                                  <tr>
                                                    ...
                                                    <td>
                                                      <input type="text" name="jumlah_beli[]" class="form-control input_isi" id="jumlah_beli_1" onkeyup="FormatCurrency(this); hitung_jumlah(1); hitung_total();" required />
                                                    </td>
                                                    <td>
                                                      <div class="input-group">
                                                        <span class="input-group-addon">Rp</span>
                                                        <input type="text" name="harga_beli[]" class="form-control input_isi" id="harga_beli_1" onkeyup="FormatCurrency(this); hitung_jumlah(1); hitung_total();" required />
                                                      </div>
                                                    </td>
                                                    <td>
                                                      <div class="input-group">
                                                        <span class="input-group-addon">Rp</span>
                                                        <input type="text" name="total_harga_beli[]" class="form-control input_isi class_harga_beli" id="total_harga_beli_1" onkeyup="FormatCurrency(this);" readonly required />
                                                      </div>
                                                    </td>
                                                    <td>
